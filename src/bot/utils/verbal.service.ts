...
import { VerbalConfig, VerbalConfigDocument } from '../../database/schemas/verbal.schema';
...
export class VerbalService {
  ...
  constructor(@InjectModel('VerbalConfig') private verbalConfigModel: Model<VerbalConfigDocument>) {
    this.verbalConfigModel.findOne({}).then(async (verbalConfig: VerbalConfig) => {
      if (!verbalConfig) {
        verbalConfig = new VerbalConfig();
        verbalConfig.keyword = VerbalService.defaultSensitiveKeyword();

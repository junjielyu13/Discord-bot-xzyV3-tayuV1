...
export class EmbedFactory {
  ...
  async getSignInCard(guildMember: GuildMember) {
    const memberDoc = await this.memberService.getMemberSafe(guildMember.id);
    const now = moment();
    const imageUrl = 'https://source.unsplash.com/1300x900?';
    let embed = new MessageEmbed();
    embed.setAuthor(guildMember.displayName + ', ' +
      getSayHello(now.hour()), guildMember.user.avatarURL());
    ...
    embed.setImage(imageUrl + getKeyword(now.hour()) + '/?sig=' + Math.random());
  ...
  async getQuotationCard(quotation?: QuotationDocument) {
    ...
    embed.setDescription(quotation.author + ` \`${quotation.id.substr(-5)}\``);
  ...
  async getContent(list: MemberDocument[], s: Predicate, count: number, key: string) {
    list.sort(s);
    let content = '';
    ...
    for (let i = 0; i < length; i++) {
      ...
      const xx = await this.xiuXianService.updateXiuXianData(memberDoc);
      content += `> \`${index}.\` \`${xx.currentMajorLevelText}-${xx.currentMinorLevelText}\`` +

...
import { DailyStudyPeriod, Member, MemberDocument, StudyPeriod } from '../../database/schemas/member.schema';
import { XiuXian } from '../../interfaces/xiu.xian';
import moment from 'moment';

const MajorLevel = ['筑基', '开光', '融合', '心动', '灵寂', '金丹', '元婴',
  '出窍', '分神', '合体', '渡劫', '大乘', '炼虚', '真仙', '金仙', '天醒', '真神', '祖仙', '返璞归真',
  '超凡入圣', '因果不沾', '圣人不死', '天道不灭'];
const MinorLevel = ['前期', '中期', '后期'];

const Experiences = [0, 36000*2, 97200*2, 194400*2, 327600*2, 496800*2, 702000*2, 943200*2,
  1220400*2, 1533600*2, 1882800*2, 2268000*2, 2689200*2, 3146400*2, 3639600*2, 4168799*2, 4794118*2, 5561176*2,
  6506576*2, 7677759*2, 9136533*2, 10963839*2, 13266246*2, 16184820*2];
...
export class XiuXianService {
  ...
  calculateXiuXianData(exp: number): XiuXian {
    ...
    result.currentMinorLevel = result.isMaxLevel ?
      2 : (Math.floor((Math.abs(actualExperience) / minorLevelRequired)) % 3);
    //设置当前等级文本
    ...
    if (!result.isMaxLevel) {
      //设置下一个的大等级
      result.nextMajorLevel =
        Math.min((result.currentMajorLevel +
          (result.currentMinorLevel == 2 ? 1 : 0)), MajorLevel.length - 1);
      //设置下一个的小等级
      result.nextMinorLevel = (result.currentMinorLevel + 1) % 3;
      //判断下一级别是不是满级
      result.isNextMaxLevel =
        (result.currentMajorLevel == (MajorLevel.length - 1) && result.currentMinorLevel == 2);
      //设置下一个级别的文本
      ...
      if (result.currentMinorLevel == 2) {
        result.totalLeft =
          (majorLevelRequired - (result.currentMinorLevel * minorLevelRequired)) -
          (actualExperience - (result.currentMinorLevel * minorLevelRequired));
      } else {
        result.totalLeft =
          minorLevelRequired - (actualExperience - (result.currentMinorLevel * minorLevelRequired));
      }
      //计算时间差

...
import { Channel, Guild, GuildMember, Role, TextChannel, VoiceChannel } from 'discord.js';
...
export class HelperService {
  ...
  async getChannel(predicate: (v: Channel) => boolean): Promise<IterableIterator<Channel>> {
    return (await this.client.defaultGuild.fetch()).channels.cache.filter(predicate).values();
  }
  ...
  async getChannelByName(name: string): Promise<Channel | null> {
    return (await this.client.defaultGuild.fetch()).channels.cache.find(cl => cl.name == name);
  }
  ...
  async getVoiceChannelMembers(channel: VoiceChannel): Promise<IterableIterator<GuildMember> | null> {
    if (channel == null) {
      return null;
    }
  ...
  async newQuotation(author: string, content: string): Promise<QuotationDocument> {
    const quotation = new Quotation();
    quotation.author = author;
    quotation.content = content;
    quotation.date = moment().toISOString();
    const quotationDoc: QuotationDocument =
      await (new this.quotationModel(quotation)).save();
  ...
  async getChannelMessageByID(channelID: string, messageID: string) {
    const channel =
      this.client.defaultGuild.channels.cache.get(channelID) as TextChannel;

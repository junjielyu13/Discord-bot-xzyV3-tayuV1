...
export type  CommandCallBack = (msg: Message, command: string[]) => Promise<Boolean>;
...
export class ConfigController {
    ...
    do {
      if (command.length > 0) {
        if (this.commandMap.has(command[0])) {
          if (!(await this.commandMap.get(command[0])(msg, command))) {
            break;
          }
        }
      }
  ...
  async removeQuotation(raw) {
    ...
    if (type == 'MESSAGE_REACTION_ADD' &&
      ...
      let message = await this.helperService.getChannelMessageByID(channelID, messageID);
      if (emj == '❌') {
        if (memberID != BotConfig.Self && message.member.id == BotConfig.Self) {
          ...
          if (message.embeds) {
            ...
            if (firstEmbed) {
              ...
              if (id) {
                let member = await this.helperService.getMemberById(memberID);
                let quotation = (await this.helperService.pickQuotation({
                  short_id: id[0],
                }))[0];
                if (quotation && member.displayName == quotation.author) {
                  await message.fetch();
                  await message.delete();
                  await quotation.delete();
                  await this.helperService.deleteQuotation(null, quotation);
                }
              }
            }
          }
  ...
  async handlerQuotation(msg: Message, command: string[]): Promise<boolean> {
    if (command[1] == 'add') {
      ...
      if (content.length > 0) {
        let quotation =
          await this.helperService.newQuotation(msg.member.displayName, content);
        await msg.reply(await this.embedFactory.getQuotationCard(quotation)).then(async m => {
          await m.react('❌');
        });
        return true;
      } else {

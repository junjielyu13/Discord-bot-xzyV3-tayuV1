        ...
        if (newChannelID != null) {
          ...
          if (isXiuXianCategory) {
            if (!online) {
              ...
              {
                const channel = await this.helperService.getTextChannelById(BotConfig.Channel.Daily);
                await channel.send(`<@!${member.id}> 签到成功`);
                await channel.send(await this.embedFactory.getSignInCard(member));
              }
              this.memberService.online.set(member.id);
            }
            isLeave = false;
          }
          await member.voice.setMute((newChannelID == BotConfig.Channel.Study));
        }
      }
